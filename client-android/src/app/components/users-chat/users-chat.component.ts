...
export class UsersChatComponent implements OnInit {
  ...
  constructor(private _webSocketService: WebSocketService, public _user: UserService, private _api: ApiService, private _toastr: ToastrService) { }
  ...
  breakFreeBook() {
    ...
    console.log(book)
    // book = this.
    // this._webSocketService.emit('book', {
    //   message: 'Libro '
    // })      
    this._api.requestBook(book, this._day, '5ecec9791ba037668c2fc64e', 'book', this._currentBorrower).subscribe(response => () => {
    })
    this.modal = false
    this._toastr.success('Prestamo realizado, ya solo te falta entregar el libro')
  }
